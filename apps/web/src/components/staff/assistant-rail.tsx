...
import type { BookingFieldKey, SeatHold } from '@ordervoice/contracts'
...
export function AssistantRail({ state, onSpeakSuggestion, onSendCustomReply, onConfirm, seatHold = null }: Props) {
  const [reply, setReply] = useState('')
  const [copied, setCopied] = useState(false)
  const gate = getBookingConfirmationState(state.booking)
  const durableHoldReady = state.booking.runtimeProfile !== 'durable' || isMatchingActiveHold(state, seatHold)
  ...
  return (
    <aside className="space-y-3" aria-label="Trợ lý nhân viên">
      ...
      <section className="rounded-[18px] border border-[var(--hairline)] bg-white p-4">
        ...
        {state.booking.runtimeProfile === 'durable' && !durableHoldReady ? (
          <div className="mt-3 flex gap-2 rounded-[10px] bg-[var(--warning-soft)] p-3 text-xs leading-5 text-[var(--warning)]">
            <WarningCircleIcon size={16} className="mt-0.5 shrink-0" weight="fill" aria-hidden />
            <span>Cần giữ đúng ghế còn hiệu lực trước khi xác nhận.</span>
          </div>
        ) : null}
      </section>
      ...
      <section className="rounded-[18px] border border-[var(--hairline)] bg-white p-4">
        {state.booking.status === 'confirmed' ? (
          ...
          <>
            ...
            <button
              type="button"
              disabled={!gate.ready || !durableHoldReady}
              onClick={onConfirm}
...
function isMatchingActiveHold(state: CallSessionState, hold: SeatHold | null): boolean {
  if (!hold || hold.status !== 'active' || Date.parse(hold.expiresAt) <= Date.now()) return false
  const tripId = state.booking.tripId ?? state.booking.selectedTrip?.id
  if (!tripId || hold.tripId !== tripId) return false
  return hold.seatCodes.length === state.booking.seats.length
    && hold.seatCodes.every((code) => state.booking.seats.includes(code))
}
