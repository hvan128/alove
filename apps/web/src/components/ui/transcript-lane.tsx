...
export function TranscriptLane({ segment }: { segment: TranscriptSegment }) {
  ...
  return (
    <article className={cn('rounded-xl border p-3', provisional ? 'border-dashed border-[var(--hairline)] bg-[var(--pearl)]' : 'border-[var(--hairline)] bg-[var(--surface)]')}>
      ...
      <p className="mt-2 font-mono text-[11px] text-[var(--muted)]">{(segment.startedAtMs / 1000).toFixed(1)} đến {(segment.endedAtMs / 1000).toFixed(1)}s · {segment.source}</p>
    </article>
  )
}
