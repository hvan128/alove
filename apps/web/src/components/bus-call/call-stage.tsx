...
function TypewriterCaption({ text, terms }: { text: string; terms: string[] }) {
  ...
  useEffect(() => {
    if (!animate) return
    const prefix = text.startsWith(previousText.current)
      ? null
      : commonPrefixLength(text, previousText.current)
    previousText.current = text
    const timer = window.setInterval(() => {
      setVisibleChars((current) => {
        const visible = prefix === null ? current : Math.min(current, prefix)
  ...
  if (!animate) return <>{renderHighlighted(text, terms)}</>
