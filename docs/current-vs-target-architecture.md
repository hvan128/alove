...
Code hiện tại có `/operations`, `/reports`, `/admin/catalog`, `/admin/vehicles`, `/call` và `/staff`. Dashboard hiển thị queue/active calls/KPI/departures; catalog có revision, validate, CSV dry-run và publish; vehicle template có visual grid 1–2 tầng; trip inventory có hold/renew/release/consume và staff-only seat picker.
...
Current UI có caller-presence indicator và Human/Auto toggle, nhưng chưa có incoming queue, notification, accept action hoặc exclusive assignment. Vì vậy không được mô tả toggle hiện tại như full staff-accept/delegation workflow.

## Ma trận main features

| Main feature | Status | Current implementation | Evidence | Missing / next proof |
|---|---|---|---|---|
| Incoming-call notification | **Chưa có** | Staff page chỉ hiển thị `Đang chờ người gọi` hoặc caller presence. | [`call-toolbar.tsx`](../apps/web/src/components/staff/call-toolbar.tsx) | Realtime incoming queue, visual/audio notification, timeout/escalation. |
| Staff nhận và sở hữu cuộc gọi | **Chưa có** | Session được mở trực tiếp bằng query code; chưa có `ringing → accepted_by_staff` command hay owner lock. | [`staff/page.tsx`](../apps/web/src/app/staff/page.tsx), [`session-state.ts`](../apps/web/src/lib/call/session-state.ts) | Nút **Nhận cuộc gọi**, atomic assignment, reassignment/audit. |
...
| Human takeover và trao lại quyền | **Một phần** | Human/Auto toggle hai chiều giữ shared state; preferences là room event. | [`call-toolbar.tsx`](../apps/web/src/components/staff/call-toolbar.tsx), [`session-state.test.ts`](../apps/web/src/lib/call/session-state.test.ts) | Session owner authorization, delegation scope, reason và dedicated audit event. |
...
| Enterprise dashboard | **Đã có baseline** | Apple-like operations landing, queue/active calls, departures/occupancy, alerts, KPI và 90-day reports; nav theo role. | [`operations-dashboard.tsx`](../apps/web/src/components/operations/operations-dashboard.tsx), [`operations-report.tsx`](../apps/web/src/components/operations/operations-report.tsx) | Realtime notification, atomic ownership, search và cross-session audit drilldown. |
...
| Persistence và audit | **Một phần** | Schema/migration/repository có; thiếu DB thì memory fallback `durable:false`. | [`session-repository.test.ts`](../apps/web/src/lib/db/session-repository.test.ts), [`schema.ts`](../db/schema.ts) | Credentialed Neon migration, transaction/retry test và retention job evidence. |
