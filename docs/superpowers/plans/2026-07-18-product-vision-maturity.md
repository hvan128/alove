# Product Vision Maturity Rewrite Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite Product Vision so current `main`, credentialed pilot and target operations are distinct, measurable maturity levels.

**Architecture:** Keep `specs/product-vision.md` as concise north-star document. Link detailed feature and evidence documents instead of duplicating acceptance matrices, and label every runtime claim by maturity level.

**Tech Stack:** Markdown, Git, repository documentation conventions

## Global Constraints

- Only `specs/product-vision.md` changes as product implementation.
- `specs/features.md` remains target feature source of truth.
- Do not claim LiveKit, VALSEA, Neon, inventory or booking issuance as live without credentialed evidence.
- Do not invent metric baselines or targets.
- Keep payment, external inventory guarantees, PSTN/SIP, Zalo raw-call audio, SMS delivery and autonomous confirmation excluded until separately verified.
- Preserve unrelated worktree changes.

---

### Task 1: Rewrite Product Vision around three maturity levels

**Files:**
- Modify: `specs/product-vision.md`
- Reference: `docs/superpowers/specs/2026-07-18-product-vision-maturity-design.md`
- Reference: `specs/features.md`
- Reference: `docs/current-vs-target-architecture.md`

**Interfaces:**
- Consumes: current implementation evidence and F-01 through F-18 target requirements.
- Produces: one product-level narrative with current, pilot and target claims clearly labeled.

- [ ] **Step 1: Replace Product Vision with approved maturity narrative**

Write this complete content to `specs/product-vision.md`:
...
- [ ] **Step 2: Review claim boundaries against target specification**

Run:

```bash
git diff -- specs/product-vision.md
```

Expected: diff distinguishes all three maturity levels; current section contains no live-provider, real-ticket or real-seat claim; target journey includes F-01, F-02, F-03, F-06 through F-16 and F-18 boundaries.

- [ ] **Step 3: Verify Markdown and source links**

Run:

```bash
rg -n "TODO|TBD" specs/product-vision.md
test -f specs/features.md
test -f docs/current-vs-target-architecture.md
git diff --check -- specs/product-vision.md
```

Expected: `rg` returns no matches; both `test` commands exit 0; `git diff --check` returns no output.

- [ ] **Step 4: Commit only planned documentation**

Run:

```bash
git add specs/product-vision.md docs/superpowers/plans/2026-07-18-product-vision-maturity.md
git diff --cached --name-only
git commit -m "docs: align product vision with maturity levels"
```

Expected staged paths:

```text
docs/superpowers/plans/2026-07-18-product-vision-maturity.md
