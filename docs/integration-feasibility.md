...
VALSEA documents a realtime WebSocket at `wss://api.valsea.ai/v1/realtime`, session start/configuration and PCM16 16 kHz mono audio with partial/final transcript events. The gateway adapter implements that shape and fixture tests validate its event mapping. A real call cannot be performed because no VALSEA sandbox credential has been provisioned in this environment. Add `VALSEA_API_KEY` server-side, then run the documented live smoke command.
...
3. Deploy the gateway behind HTTPS/WSS and set `PUBLIC_GATEWAY_URL` on Vercel.
