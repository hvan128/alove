...
## Gate trước khi đổi nhãn provider thành live

1. Đặt credentials trong secret owner đúng runtime; scan source và log để xác nhận redaction.
2. Chạy consented smoke tại runtime dự kiến; ghi ngày, source SHA, deployment ID, account mode, region, correlation ID, latency và failure behavior.
3. Test malformed, duplicate, replay input và provider disconnect; xác nhận fallback và không tạo booking thừa.
4. Với Neon, chạy migration cùng confirmation transaction tests. Với telephony, phải có cuộc gọi media thật đã được phép.
5. Giữ nhãn `code seam`, `pilot` hoặc `unavailable` đến khi đủ evidence; không suy luận inventory, payment, SMS, PSTN hoặc realtime production từ adapter.
