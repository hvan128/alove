# Runbook: nối số điện thoại vào agent VéĐi qua LiveKit SIP
...
Luồng đích: khách gọi số DID → trunk provider chuyển SIP về LiveKit → dispatch rule tạo room `booking-<id>` và dispatch agent `vedi` → agent chào, đặt vé qua `@ordervoice/core` → dashboard `/dashboard` xem transcript live, booking lưu Neon.

## 0. Điều kiện có sẵn

- LiveKit Cloud project (đã có) + `lk` CLI đã đăng nhập: `lk cloud auth`.
- Agent worker deploy và đăng ký tên `vedi` (xem `agent/README.md`). Worker local dev tự đổi tên thành `vedi-dev` nên KHÔNG nhận cuộc gọi prod.
...
> giá trước (xem `docs/integration-feasibility.md`).
...
```bash
cp scripts/sip/inbound-trunk.example.json /tmp/vedi-inbound-trunk.json
# sửa "numbers" thành số DID thật (định dạng E.164, ví dụ +15105550123)
lk sip inbound create /tmp/vedi-inbound-trunk.json
```
...
Rule này: mỗi cuộc gọi vào → room riêng `booking-<random>` → dispatch agent `vedi`.
...
Sau cuộc gọi thật đầu tiên, cập nhật `docs/integration-feasibility.md` với: ngày,
provider, region số, latency chào + latency mỗi lượt, chất lượng ASR tiếng Việt
trên audio điện thoại (8 kHz) và các lỗi gặp. Chỉ sau đó mới đổi nhãn UI/docs sang
"phone live".
...
| Kết nối nhưng im lặng | Agent worker có đang chạy và đăng ký đúng tên `vedi` không (`lk agent list`) |
